...
import org.scalastuff.scalabeans.{ Metamodel, ValueMetamodel, ContainerMetamodel, ConvertedMetamodel, BeanDescriptor }
...
object ValueHandler {
  def apply(metamodel: Metamodel): Option[ValueHandler] = metamodel.visibleMetamodel match {
      ...
      cm.scalaType match {
        case at @ ArrayType(_) =>
          for (valueHandler <- ValueHandler(cm.elementMetamodel))
            yield new ArrayValueHandler(WrappedValueHandler(valueHandler, cm.elementMetamodel)) {
            def newBuilder = at.newArrayBuilder[V].asInstanceOf[CB]
          }

        case OptionType(_) =>
          for (valueHandler <- ValueHandler(cm.elementMetamodel))
            yield new OptionValueHandler(WrappedValueHandler(valueHandler, cm.elementMetamodel))

        case t @ TraversableType(_) => t.newBuilder match {
          case Some(newBuilderFunction) =>
            for (elemValueHandler <- ValueHandler(cm.elementMetamodel))
              yield new CollectionValueHandler(WrappedValueHandler(elemValueHandler, cm.elementMetamodel)) {
    ...
    case bd: BeanDescriptor => Some(BeanValueHandler(bd))
